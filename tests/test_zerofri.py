...
class TestZeroFRI(TestCase):
    def test_zerofri_batch(self):
        ...
        for i in range(2, 5):
            ...
            proof = ZeroFRI.prove_zerofri(mle_poly, rate, point, v, primitive_element, FIELD_SIZE, MerlinTranscript(b'zeromorph'), one=BabyBear.one(), debug=0, batch=True)
            ...
            ZeroFRI.verify_zerofri(f_proof, f_val, quotients_proof, quotients_vals, num_vars, rate, point, v, primitive_element, FIELD_SIZE, MerlinTranscript(b'zeromorph'), one=BabyBear.one(), debug=0, batch=True)
    ...
    def test_zerofri_not_batch(self):
        ...
        for i in range(2, 5):
            ...
            proof = ZeroFRI.prove_zerofri(mle_poly, rate, point, v, primitive_element, FIELD_SIZE, MerlinTranscript(b'zeromorph'), one=BabyBear.one(), debug=0, batch=False)
            print(f"size of not batched proof for {num_vars} variables:", getsizeof(dumps(proof)))
            f_proof, f_val, quotients_proof, quotients_vals = proof
            ZeroFRI.verify_zerofri(f_proof, f_val, quotients_proof, quotients_vals, num_vars, rate, point, v, primitive_element, FIELD_SIZE, MerlinTranscript(b'zeromorph'), one=BabyBear.one(), debug=0, batch=False)
