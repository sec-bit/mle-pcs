...
/// Neil: Error type for mmcs verifying
#[derive(Debug)]
pub enum MerkleTreeError {
    /// Neil: opened_values isn't correct
    WrongBatchSize,
    /// Neil: Disabled for now since TwoAdicFriPcs and CirclePcs currently pass 0 for width.
    WrongWidth,
    /// Neil: proof.len() != log_max_height
    WrongHeight {
        max_height: usize,
        num_siblings: usize,
    },
    /// Neil: we receive wrong root
    RootMismatch,
}
    ...
    [PW::Value; DIGEST_ELEMS]: Serialize + for<'de> Deserialize<'de>,
{
    ...
    ///
    /// # Returns
    /// A tuple containing the leaves data and the merkle path
    fn open_batch<M: Matrix<P::Value>>(
        ...
        let openings = prover_data
            ...
            .map(|matrix| {
                ...
                // Neil: get the row of the matrix
    ...
    fn verify_batch(
        ...
        // Neil: root 是 merkle path 上子树的根节点哈希值，初始值为 opened_values
