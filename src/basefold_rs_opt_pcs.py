...
class DEEPFOLD_RS_PCS:
    ...
    def verify_queries(self, 
        ...
        for i in range(self.max_queries_try):
            new_query = tr.squeeze(int, b"query", 24) % (N//2)
            ...
            if len(queries) >= self.num_queries:
                break
        if self.debug > 0:
            print(f"V> len(code)={N}, queries={self.num_queries}")
    ...
    def verify_eval(self, 
        ...
        self.verify_queries(f0_code_len, r_vec, [f_cm.cm]+code_commitments, final_constant, tr, 
            query_paths, merkle_paths)
