...
class BASEFOLD_PCS:
    ...
    def prove_eval(self, 
        ...
        f_code_cm_vec = []
        f_code = f0_code
        for i in range(k - log_2(self.encoder.k0)):
            if self.debug > 0: 
            ...
            if i == k - log_2(self.encoder.k0) - 1:
                f_code_copy = f_code.copy()
                if isinstance(self.encoder, FoldableRSCoder):
                    for j in range(log_2(self.encoder.k0)):
                        f_code_copy = self.fold_with_multilinear_basis(f_code_copy, r)
                    for j in range(1, self.encoder.blowup_factor):
                        assert f_code_copy[j] == f_code_copy[j - 1], \
                            f"f_code_copy[{j}] != f_code_copy[{j - 1}], f_code_copy={f_code_copy}"
                else:
                    for j in range(self.encoder.blowup_factor - 1):
                        for i in range(self.encoder.k0):
                            assert f_code_copy[j * self.encoder.k0 + i] == f_code_copy[j * self.encoder.k0 + i + self.encoder.k0], \
                                f"f_code_copy[{j * self.encoder.k0 + i}] != f_code_copy[{j * self.encoder.k0 + i + self.encoder.k0}], f_code_copy={f_code_copy}"
                tr.absorb(b"f_final_code", f_code)
            else:
            ...
            if self.debug > 1:
                ...
                assert f_code == debug_f_folded_code, f"Enc(fold(message)) != fold(Enc(message)), f_code={f_code}, debug_f_folded_code={debug_f_folded_code}"
        ...
        if self.debug > 1:
            f_copy = f.copy()
            coded = self.encoder.generator_func(f_copy, self.encoder.k0, self.encoder.blowup_factor)
            assert coded == f_code, \
                f"coded != f_code, coded = {coded}, f_code = {f_code}"
    ...
    def verify_eval(self, 
        ...
        final_code = proof['final_code']
        final_f = proof['final_f']
        ...
        for i in range(k - log_2(self.encoder.k0)):
            ...
            if i == k - log_2(self.encoder.k0) - 1:
                tr.absorb(b"f_final_code", final_code)
            else:
        ...
        #     print(f"V> eq_eval = {eq_eval}, final_code[0] = {final_code[0]}, sumcheck_sum = {sumcheck_sum}")
        #     assert eq_eval * final_code[0] == sumcheck_sum, \
        #         f"eq_eval * final_code[0] != sumcheck_sum, eq_eval = {eq_eval}, final_code[0] = {final_code[0]}, sumcheck_sum = {sumcheck_sum}"
        #     print(f"V> verify sumcheck_sum passed ")

        # checked01 = eq_eval * final_code[0] == sumcheck_sum
        encoded = self.encoder.generator_func(final_f, self.encoder.k0, self.encoder.blowup_factor)
        checked01 = encoded == final_code, f"encoded != final_code, encoded = {encoded}, final_code = {final_code}"
        ...
        checked02 = self.verify_queries(queries, k - log_2(self.encoder.k0), f_cm.root, f_code_cm_vec, final_code, 
        ...
        for i, (indices, cur_query) in enumerate(zip(query_indices, query_paths)):
            ...
            for j, idx in enumerate(indices):
                ...
                if j < len(indices) - 1:
                    ...
                    assert final_code[next_index] == folded_value, f"i={i}, j={j}, folded = {folded_value}, half = {half}, next_index = {next_index}, next = {next}"
            if self.debug > 0:
...
def test_prove_verify():
    ...
    log_k0 = 3
    depth = log_n - log_k0
    encoder = FoldableRSCoder(k0=2 ** log_k0, depth=depth, c=8)
    ...
    evals = [Fp.random() for _ in range(1 << log_n)]
...
def test_prove_verify_random():
    ...
    log_k0 = 1
    depth = log_n - log_k0
    encoder = FoldableCoder(k0=2 ** log_k0, depth=depth, c=2 ** blowup_factor_log2)
